...
const NotificationDropdown: React.FC = () => {
  const notifications = [
    ...
    },
    {
      ...
      time: "Hôm nay - 9:42",
      icon: <GasStationIcon className="w-5 h-5 text-orange-500" />, 
    ...
    },
    {
      ...
      time: "Hôm nay - 9:42",
      icon: <DollarLineIcon className="w-5 h-5 text-purple-500" />,
  ...
  return (
    <div className="absolute top-full right-0 md:left-1/2 md:-translate-x-1/2 mt-2 w-[360px] max-w-[90vw] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
      {/* Header */}
      <div className="px-5 py-4 flex items-center justify-between border-b border-gray-50 dark:border-gray-700">
        ...
        <button className="p-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md transition-colors">
      ...
      <div className="max-h-[380px] overflow-y-auto scrollbar-thin">
        {notifications.map((noti) => (
          <div key={noti.id} className="px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/40 cursor-pointer transition-colors flex gap-4 border-b border-gray-50 dark:border-gray-700/50 last:border-0 relative">
            <div className={`flex-shrink-0 w-10 h-10 rounded-full ${noti.iconBg} flex items-center justify-center`}>
              {noti.icon}
            </div>
            <div className="flex-grow min-w-0">
              <div className="flex items-center justify-between mb-0.5 mt-0.5">
                <p className="text-sm font-bold text-gray-800 dark:text-gray-200 truncate pr-4">{noti.title}</p>
                {noti.unread && <div className="absolute right-5 top-8 w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>}
              </div>
