    ...
    >
      ...
      <div className="px-5 py-4 flex items-center justify-between border-b border-gray-50 dark:border-gray-700">
        <h3 className="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
          Thông báo
          <span className="px-2 py-0.5 bg-orange-500 text-white text-theme-tiny rounded-full">
            9
          </span>
        </h3>
        <button className="flex-shrink-0 p-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md transition-colors rotate-90">
      ...
      <div className="max-h-[380px] overflow-y-auto scrollbar-thin">
        {notifications.map((noti) => (
          <div
            key={noti.id}
            className="px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/40 cursor-pointer transition-colors flex gap-4 border-b border-gray-50 dark:border-gray-700/50 last:border-0 relative"
          >
            <div
              className={cn(
            ...
            <div className="flex-grow min-w-0">
              <div className="flex items-start justify-between mb-0.5 mt-0.5 gap-2">
                <p className="text-sm font-bold text-gray-800 dark:text-gray-200 pr-4 break-words">
                  {noti.title}
                </p>
                {noti.unread && (
                  <div className="flex-shrink-0 right-5 top-8 w-2 h-2 bg-blue-500 rounded-full shadow-soft mt-1.5"></div>
                )}
              </div>
