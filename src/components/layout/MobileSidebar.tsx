                    ...
                    <div key={item.id} className="space-y-0.5">
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-bold transition-all ${
                          activeTabId === item.id || (item.subItems && item.subItems.some(s => s.id === activeTabId))
                      ...
                      >
                        ...
                        {item.subItems && (
                          <ChevronDownIcon className={`w-4 h-4 transition-transform duration-300 ${isExpanded(item.id) ? "rotate-180" : ""}`} />
                        )}
                      </button>
                      
                      {/* Sub-items with Collapse Animation */}
                      {item.subItems && (
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded(item.id) ? "max-h-[500px] opacity-100 mt-1" : "max-h-0 opacity-0"}`}>
