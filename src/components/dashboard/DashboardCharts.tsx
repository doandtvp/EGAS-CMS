...
const RevenueGrowthChart: React.FC = () => {
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: ["06/09", "07/09", "08/09", "09/09", "10/09", "11/09", "12/09"],
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] border border-grayscale-10 dark:border-gray-700/50 shadow-dashboard col-span-12 lg:col-span-4">
      ...
      <div className="flex items-center gap-3 mb-6">
        <span className="text-theme-2xl font-bold text-blue-custom leading-none">
          920 triệu VNĐ
        </span>
        <div className="flex items-center justify-center min-w-[66px] h-[31px] bg-green-custom-100 border border-green-custom-200 px-2 rounded-full">
          <span className="text-[16px] font-semibold text-green-custom-600 leading-[24px]">
            10%
          </span>
          <span className="text-[14px] ml-1 font-semibold text-green-custom-600 leading-none mt-0.5">
            ↑
          </span>
        </div>
        <span className="text-[14px] font-normal text-grayscale-50 leading-[20px]">
          + 120 triệu so với tuần trước
        </span>
      </div>

      <div className="h-[250px] relative">
        <div className="absolute top-[-20px] left-[-10px] text-[10px] text-gray-400 font-medium">
          Triệu vnđ
        </div>
...
const RevenueByShiftChart: React.FC = () => {
  const options: ApexOptions = {
    ...
    colors: ["#3B82F6"],
    xaxis: {
      categories: ["Ca sáng", "Ca chiều", "Ca tối"],
      axisBorder: { show: false },
      axisTicks: { show: false },
      labels: { style: { colors: "#94A3B8", fontSize: "11px" } },
    },
